...
# Khung hiển thị video từ camera và vùng cắt ảnh sinh viên
...
# Biến toàn cục cho vùng ảnh trích xuất
imgGetText = None
image1 = None
image2 = None
...
# phan 2
def display():
    """Hiển thị hình ảnh từ camera và cắt vùng ảnh sinh viên"""
    ...
    canvas.image = image1  # Giữ tham chiếu để tránh bị xóa bởi garbage collector

    # Xử lý ảnh để tìm contour của thẻ sinh viên
    blurred = cv2.GaussianBlur(frame, (5, 5), 0)
    edges = cv2.Canny(blurred, 75, 150)
    contours, _ = cv2.findContours(edges, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    
    # Tìm và cắt vùng thẻ sinh viên
