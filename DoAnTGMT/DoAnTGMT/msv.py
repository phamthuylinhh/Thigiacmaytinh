...
def txtt():
    ...
    NGANH = roi_resized[115:138,118:238]
    KHOA = roi_resized[135:158,80:238]
    KHOAS = roi_resized[155:176,120:238]
    MSV = roi_resized[192:215,237:315]
    ...
    imCloseNGANH = cv2.morphologyEx(imDialNGANH, cv2.MORPH_OPEN, kernel)
    imCloseKHOA = cv2.morphologyEx(imDialKHOA, cv2.MORPH_OPEN, kernel)
    imCloseKHOAS = cv2.morphologyEx(imDialKHOAS, cv2.MORPH_OPEN, kernel)
    imCloseTEN = cv2.morphologyEx(imDialTEN, cv2.MORPH_CLOSE, kernel)

    cv2.imshow('ten', imDialTEN)
    cv2.imshow('ng', imCloseNGANH)
    cv2.imshow('kh', imCloseKHOA)
    cv2.imshow('ks', imCloseKHOAS)
    cv2.imshow('ms', imCloseMSV)
    
    textTEN = pytesseract.image_to_string((thresTEN), lang='vie')
    textNGANH = pytesseract.image_to_string((imCloseNGANH), lang='vie')
    textKHOA = pytesseract.image_to_string((imCloseKHOA), lang='vie')
    textKHOAS = pytesseract.image_to_string((imCloseKHOAS), lang='vie')
    textMSV = pytesseract.image_to_string((imCloseMSV))
