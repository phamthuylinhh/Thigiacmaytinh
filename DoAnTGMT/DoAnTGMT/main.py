...
def txtt():
    ...
    HOTEN = roi_resized[92:118, 90:214]
    NGANH = roi_resized[115:138,112:238]
    KHOA = roi_resized[135:158,80:238]
    KHOAS = roi_resized[155:176,120:238]
    MSV = roi_resized[190:215,238:315]

    # cv2.imshow('a', MSV)

    HOTEN = cv2.resize(HOTEN, None, fx=2.5, fy=2.5, interpolation=cv2.INTER_CUBIC)
    NGANH = cv2.resize(NGANH, None, fx=2.5, fy=2.5, interpolation=cv2.INTER_CUBIC)
    KHOA = cv2.resize(KHOA, None, fx=2.5, fy=2.5, interpolation=cv2.INTER_CUBIC)
    KHOAS = cv2.resize(KHOAS, None, fx=2.5, fy=2.5, interpolation=cv2.INTER_CUBIC)
    MSV = cv2.resize(MSV, None, fx=3, fy=3, interpolation=cv2.INTER_CUBIC)
    ...
    ret, threshTEN = cv2.threshold(grayTEN, 170, 255, cv2.THRESH_BINARY)
    ret, threshNGANH = cv2.threshold(grayNGANH, 170, 255, cv2.THRESH_BINARY)
    ret, threshKHOA = cv2.threshold(grayKHOA, 170, 255, cv2.THRESH_BINARY)
    ret, threshKHOAS = cv2.threshold(grayKHOAS, 170, 255, cv2.THRESH_BINARY)
    threshMSV = cv2.adaptiveThreshold(grayMSV,255,cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY,9,2)

    # kernel = np.ones((3,3), np.uint8)
    # dilateTEN = cv2.dilate(threshTEN,kernel,iterations = 1)
    # dilateNGANH = cv2.dilate(threshNGANH,kernel,iterations = 1)
    # dilateKHOA = cv2.dilate(threshKHOA,kernel,iterations = 1)
    # dilateKHOAS = cv2.dilate(threshKHOAS,kernel,iterations = 1)
    # dilateMSV = cv2.dilate(threshMSV,kernel,iterations = 1)

    
    # cv2.imshow('a', threshMSV)

    textTEN = pytesseract.image_to_string((threshTEN), lang='vie')
    textNGANH = pytesseract.image_to_string((threshNGANH), lang='vie')
    textKHOA = pytesseract.image_to_string((threshKHOA), lang='vie')
    textKHOAS = pytesseract.image_to_string((threshKHOAS), lang='vie')
    textMSV = pytesseract.image_to_string((threshMSV), lang='vie')
